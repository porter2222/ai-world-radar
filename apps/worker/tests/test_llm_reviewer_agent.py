...
def rich_detail_body() -> str:
    ...
    """
    return (
        "Hacker News 上出现一则围绕 OpenAI 新编码 Agent 的讨论，开发者正在关注它是否会改变日常软件开发流程。"
        "这件事把 AI 编程工具从代码补全推进到任务协作层面，讨论重点也从“能不能写代码”转向“能不能参与工程流程”。\n\n"
        "这类编码 Agent 的讨论通常围绕任务执行能力展开，包括理解项目上下文、跨文件修改、生成测试、定位错误和辅助代码审查。"
        "如果这些能力变得稳定，它们可能改变开发者和 AI 工具之间的协作方式。\n\n"
        "社区分歧主要集中在可靠性、控制权和审查成本。一部分开发者期待减少重复劳动，另一部分开发者担心 Agent 误解需求、引入隐蔽 bug 或增加维护负担。"
        "这些分歧是详情页必须呈现的信息量，也能说明热议背后的真实采用门槛并不只是模型是否足够聪明。\n\n"
        "对中文用户来说，这件事可以帮助判断 AI 编程工具下一阶段的竞争重点：模型能力、工程上下文、权限边界、团队协作和真实项目成功率。"
        "这些因素比单一演示视频更能影响工具是否真正进入生产流程，也会影响个人学习路线、团队试点范围和代码审查习惯。\n\n"
        "后续值得观察的是它能否在真实项目里稳定完成测试补全、问题修复、迁移脚本和代码审查辅助。"
        "如果工具能提供清晰修改记录、可复现测试结果和可控权限，它才更可能从新鲜概念进入团队日常流程。"
    )
