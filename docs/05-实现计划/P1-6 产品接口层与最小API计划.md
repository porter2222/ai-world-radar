...
- 前台事件列表返回 `source_hint` 和 `source_count` 供首页卡片单行展示来源；`source_count` 按平台去重，不按 `source_refs` 原始条数计数。例如两个 HN 链接加一个 GitHub 链接，列表返回 `source_hint="Hacker News 等 2 源"`、`source_count=2`。
- 前台事件列表不返回完整 `source_refs`，详情页继续由 `GET /events/{slug}` 返回完整来源引用。
...
- 全链路真实验收（2026-06-23）：live HN API 采集到 `sig_d4823864c09a4fc981daec6d6cc3f41d`，标题为 `Show HN: PMB – local-first memory for AI coding agents over MCP`；真实 LLM pipeline 写入 PostgreSQL，`run_id=run_f2cabe57ea404c5fbb23244d9a428f22`、`published_event_id=pub_bc6ee13a869347c4b3abd083e2e73e42`、slug 为 `pmb-local-first-memory-mcp-ai-coding-agents`；FastAPI `GET /events`、`GET /events/{slug}`、`GET /admin/pipeline-runs/{run_id}`、`GET /admin/pipeline-runs/{run_id}/agent-runs` 均能读取本轮真实产物。
- 前端首页来源提示补充（2026-06-23）：`GET /events` 列表 item 新增 `source_hint` 和 `source_count`，用于首页事件卡右侧单行展示来源；`source_count` 按平台去重，`GET /events` 仍不返回完整 `source_refs` 和内部评分字段，`GET /events/{slug}` 继续返回完整 `source_refs`。RED 命令为 `.\.venv\Scripts\python.exe -m pytest tests/test_product_query_service.py::test_list_published_events_returns_single_line_source_hint_with_deduped_platform_count tests/test_product_query_service.py::test_list_published_events_returns_single_source_hint_and_empty_source_state -v`，真实结果为 `2 failed in 0.88s`，失败点是 `KeyError: 'source_hint'`；实现后联合回归 `.\.venv\Scripts\python.exe -m pytest tests/test_product_api.py tests/test_product_query_service.py -v` 真实结果为 `8 passed in 1.67s`，worker 全量回归为 `122 passed in 39.51s`。
...
- 已发布事件列表返回首页卡片可用的 `source_hint` / `source_count`，但不暴露完整 `source_refs`。
...
P1-6 后端产品接口层已完成最小闭环：查询服务、响应 schema、FastAPI 只读 HTTP endpoint、脚本化 PostgreSQL 只读 smoke、真实 HTTP 运行验收、全链路真实验收和验收文档均已落地。当前可以进入产品页面联调或前端页面开发，但公开部署前仍必须补 API 鉴权和管理员权限。
