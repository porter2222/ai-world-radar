...
- 物理删除旧 `scripts/run_hn_pipeline.py`、旧 HN pipeline 和旧 Agent stub，避免后续代理误用旧链路。
...
- 旧 `scripts/run_hn_pipeline.py`、旧 `worker/pipelines/hn_event_pipeline.py` 和旧 Agent stub 已物理删除。
- worker 全量 pytest 通过。
...
用户 review 后要求：不要只做 legacy guard，要在当前分支上直接删除旧代码，让后续重构从干净的新工作区开始。

执行记录：2026-06-13 已完成物理清理。删除范围包括：

- `apps/worker/scripts/run_hn_pipeline.py`
- `apps/worker/worker/pipelines/__init__.py`
- `apps/worker/worker/pipelines/hn_event_pipeline.py`
- `apps/worker/worker/agents/evidence_agent.py`
- `apps/worker/worker/agents/event_cluster_agent.py`
- `apps/worker/worker/agents/ranking_agent.py`
- `apps/worker/worker/agents/detail_writer_agent.py`
- `apps/worker/worker/agents/brief_writer_agent.py`
- `apps/worker/worker/agents/quality_gate_agent.py`
- `apps/worker/worker/legacy/README.md`
- `apps/worker/tests/test_legacy_boundaries.py`
- `apps/worker/tests/test_legacy_entrypoints.py`

新增 `apps/worker/tests/test_legacy_runtime_cleanup.py` 作为回归测试，断言旧 runtime 文件不存在，并扫描 `worker/` 源码不再引用旧事件模型契约名。`worker/reports/pipeline_report_writer.py` 也从旧 `EvidenceCard / EventCluster / BriefItem` 统计改为新版 `signals / candidates / dossiers / published / failed / agent_runs` 统计。

真实命令结果：

- RED：`.\.venv\Scripts\python.exe -m pytest tests/test_legacy_runtime_cleanup.py tests/test_pipeline_report_writer.py -v`，删除前 `3 failed in 0.35s`。
- GREEN：同一命令在删除和改写后为 `3 passed in 0.15s`。
- 全量回归：`.\.venv\Scripts\python.exe -m pytest -v` 最终复验为 `41 passed in 4.09s`。
- cleanup smoke：`scripts/run_event_pipeline.py --database-url "sqlite+pysqlite:///scratch/p1_2_cleanup_smoke.sqlite" --create-schema-for-smoke --seed-demo-signal --run-key "manual-p1-2-cleanup-smoke"` 返回 `status=succeeded`、`published_count=1`。
- runtime 扫描：`rg -n "EvidenceCard|EventCluster|ContentArtifact|QualityGateResult|BriefItem|Brief\b|HNEventPipeline" apps\worker\worker apps\worker\scripts -g "*.py"` 无匹配。

## 16. 执行交接

P1-2 已完成，且旧可执行代码已物理删除。后续代理应把本文档作为 LangGraph 最小闭环的实现与验收记录，不要从 Task 1 重做，也不要恢复旧 `EvidenceCard / EventCluster / Brief` 主链路。
...
Plan complete, accepted, and cleaned in `docs/05-实现计划/P1-2 LangGraph工作流与三Agent最小闭环计划.md`.
