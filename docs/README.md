...
> P1-10 首页时效窗口已完成；P1-11 当前建议先落地采集层 8 小时窗口，避免 `source_signals` 继续混入明显过期来源信号。前端公共首页和事件详情页可继续基于真实 FastAPI Product API 做页面级联调截图和视觉微调。论文源、X、Reddit、YouTube、强反爬源、DailyEdition、历史归档和队列调度器继续后置。
